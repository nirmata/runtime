...
| [detect-mcp-servers](detect-mcp-servers/) | Report every stdio MCP server a workload launches, identified by package rather than by binary | `exec` deny over everything, narrowed by a `monitorFilter` over argv | monitor | BPF-LSM |
