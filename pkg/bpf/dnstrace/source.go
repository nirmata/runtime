...
// The kernel object for _cprog/dns.bpf.c is not generated in this repository:
// bpf2go needs clang, libbpf headers and a BTF-derived vmlinux.h, none of which
// exist on the development host this package was written on. Until the
// generation runs on a linux toolchain the directive below stays commented and
// NewSource reports runtimeevent.ErrSourceNotWired.
//
// TODO(#63): uncomment, run `go generate ./pkg/bpf/...` on linux, and replace
// notWired with a real ringbuf reader that calls DecodeDNSEvent on every record.
//
// //go:generate go tool bpf2go dnsTrace ./_cprog/dns.bpf.c -- -I./_cprog

// SourceName is the name this source reports to the collector. It is also the
// label used for per-source drop metrics.
const SourceName = "dnstrace"

// NewSource returns the DNS query source.
//
...
func (s notWired) Run(context.Context, chan<- runtimeevent.Event) error {
	s.log.V(2).Info("dns source has no kernel bindings on this build", "source", SourceName)
