...
/* Key of the per-cgroup observation maps: one counter per (path, decision).
 *
 * sizeof(struct path_event_key) is 132 with NO padding: MAX_PATH_LEN is 128, a
 * multiple of 4, so the __u32 decision is already naturally aligned and the
 ...
 * (path, decision) pair hash to distinct entries, producing phantom keys and
