...
// The kernel object for _cprog/exec.bpf.c is not generated in this repository:
...
// with a real ringbuf reader that calls DecodeExecEvent on every record.
//
// //go:generate go tool bpf2go execTrace ./_cprog/exec.bpf.c -- -I./_cprog/include -I./_cprog

// SourceName is the name this source reports to the collector. It is also the
// label used for per-source drop metrics.
const SourceName = "exectrace"

// NewSource returns the process-execution source.
//
...
func (s notWired) Run(context.Context, chan<- runtimeevent.Event) error {
	s.log.V(2).Info("exec source has no kernel bindings on this build", "source", SourceName)
