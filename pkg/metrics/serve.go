...
// Serve starts an HTTP server exposing g at "/metrics" on addr. It blocks
// until ctx is canceled or the server fails to serve, then shuts the
// server down and returns. A nil error is returned for both a clean
// shutdown and a graceful listener close.
func Serve(ctx context.Context, addr string, g prometheus.Gatherer, log logr.Logger) error {
	ln, err := net.Listen("tcp", addr)
	if err != nil {
		return fmt.Errorf("metrics: listen on %s: %w", addr, err)
	}
	log.V(2).Info("metrics server listening", "addr", ln.Addr().String())
	return serve(ctx, ln, g, log)
}
...
func serve(ctx context.Context, ln net.Listener, g prometheus.Gatherer, log logr.Logger) error {
