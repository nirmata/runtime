...
func TestPodCreated_MatchingAndNonMatchingPolicies(t *testing.T) {
	h := newHarness(t)
	if err := h.l.RuntimePolicyEvent(result("rpWeb", "enforce", selFor(map[string]string{"app": "web"}),
		pair(nil, []string{"/etc/shadow"}), pair(nil, []string{"/bin/sh"})), events.EventTypeCreate); err != nil {
		t.Fatal(err)
	}
	if err := h.l.RuntimePolicyEvent(result("rpDb", "enforce", selFor(map[string]string{"app": "db"}),
	...
	// both prog types of the matching policy get the pod's cgids
...
// the manager snapshots pod labels at creation time and podUpdated only reacts to
// cgid changes, so a relabel alone neither re-evaluates selectors nor touches the
// enforcers. re-evaluation happens on the next policy update.
func TestPodUpdated_LabelChangeAloneIsNotReEvaluated(t *testing.T) {
	h := newHarness(t)
	...
	if err := h.l.RuntimePolicyEvent(result("rpWeb", "enforce", sel, files, nil), events.EventTypeCreate); err != nil {
		t.Fatal(err)
	}
	f := h.enf("rpWeb", open)
	...
	if len(f.addCgids) != 0 {
		t.Errorf("relabel triggered AddCgids %v", f.addCgids)
	}
	if got := attachedPodUIDs(h.l.lsmAttachments["rpWeb"]); len(got) != 0 {
		t.Errorf("attached pods = %v, want none (labels are snapshotted at pod creation)", got)
	}
	if got := h.l.pods["podA"].labels["app"]; got != "db" {
		t.Errorf("stored label app = %q, want the snapshot %q", got, "db")
	}
	assertInvariant(t, h.l)
}
...
func TestPolicyEvents_CgidFailuresAreNonFatal(t *testing.T) {
	...
	files := pair(nil, []string{"/etc/shadow"})
	if err := h.l.RuntimePolicyEvent(result("rpWeb", "enforce", selFor(map[string]string{"app": "web"}), files, nil), events.EventTypeCreate); err != nil {
	...
	if err := h.l.RuntimePolicyEvent(result("rpWeb", "enforce", selFor(map[string]string{"app": "db"}), files, nil), events.EventTypeUpdate); err != nil {
