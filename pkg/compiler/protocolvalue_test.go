...
func TestParseProtocolValue(t *testing.T) {
	tests := []struct {
		...
		{name: "http/2 is one token, not http with ALPN 2", in: "http/2", wantProto: ProtocolHTTP2},
		{name: "dns", in: "dns", wantProto: ProtocolDNS},
		...
		{name: "unknown is not a token", in: "unknown", wantErr: ErrNotAProtocolValue},
		{name: "unclassified is observation vocabulary, not a value", in: ProtocolUnclassified, wantErr: ErrNotAProtocolValue},
		{name: "h2c is not a token, the cleartext form is http/2", in: "h2c", wantErr: ErrNotAProtocolValue},
		...
		{name: "bare http is not a token", in: "http", wantErr: ErrNotAProtocolValue},
