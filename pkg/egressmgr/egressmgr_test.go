...
func TestFullLifecycleLeavesNoDanglingState(t *testing.T) {
	...
	// 5. pod-web is relabelled. the manager only re-evaluates selectors on pod
	// create, so the informer's delete/create pair is the path that rebuilds the
	// attachment state.
	if err := e.PodEvent(makePod("pod-web", webLabels), nil, events.EventTypeDelete); err != nil {
		t.Fatalf("delete pod-web: %v", err)
	}
	web = addPod(t, e, "pod-web", apiLabels, "/cg/web")
...
func TestConcurrentPodAndPolicyEventsKeepStateConsistent(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < n; i++ {
			...
			if err := e.RuntimePolicyEvent(rp(uid, "enforce", webLabels, ips, []string{"*"}), events.EventTypeUpdate); err != nil {
